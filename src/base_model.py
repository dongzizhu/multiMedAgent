...
class BAN_Model(nn.Module):
    def __init__(self, w_emb, q_emb, v_att, b_net, q_prj, c_prj, classifier, counter, args, maml_v_emb, ae_v_emb):
        ...
        if args.autoencoder:
            ...
            # self.convert = nn.Linear(1048576, args.feat_dim)
    def forward(self, v, q):
        ...
        if self.args.maml: # get maml feature
            ...
            if len(self.args.maml_nums) > 1:
                # print(self.maml_v_emb[0](v[0]).unsqueeze(1))
                # print(self.maml_v_emb[0](v[0]).unsqueeze(1))
                maml_v_emb = self.maml_v_emb[0](v[0]).unsqueeze(1)
                # print('maml', 0)
                # print(maml_v_emb.shape, maml_v_emb)
...
class SAN_Model(nn.Module):
    def __init__(self, w_emb, q_emb, v_att, classifier, args, maml_v_emb, ae_v_emb):
        ...
        if args.autoencoder:
            self.ae_v_emb = ae_v_emb
            # self.convert = nn.Linear(16384, args.feat_dim)
            self.convert = nn.Linear(4000000, args.feat_dim)
    def forward(self, v, q):
        """Forward
...
def build_BAN(dataset, args, priotize_using_counter=False):
    ...
    if args.maml and args.autoencoder:
        return BAN_Model(w_emb, q_emb, v_att, b_net, q_prj, c_prj, classifier, counter, args, maml_v_emb,
                         ae_v_emb)
    elif args.maml:
        return BAN_Model(w_emb, q_emb, v_att, b_net, q_prj, c_prj, classifier, counter, args, maml_v_emb,
                         None)
    elif args.autoencoder:
        return BAN_Model(w_emb, q_emb, v_att, b_net, q_prj, c_prj, classifier, counter, args, None,
                         ae_v_emb)
    return BAN_Model(w_emb, q_emb, v_att, b_net, q_prj, c_prj, classifier, counter, args, None, None)
