...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, args, dictionary, dataroot='data', question_len=12):
        ...
        if args.maml:
            ...
            print('loading MAML image data from file: '+ images_path + ' ' + str(args.load))
            if args.load:
                self.maml_images_data = cPickle.load(open(images_path, 'rb'))
                for i in range(len(self.maml_images_data)):
                    self.maml_images_data[i] = my_preprocess(self.maml_images_data[i])
            else:
                self.maml_images_data = images_path
        ...
        if args.autoencoder:
            ...
            if 'RAD' in self.args.VQA_dir:
                ...
                # images_path = os.path.join(dataroot, 'pytorch_images1024_ae.pkl')
                images_path = os.path.join(dataroot, 'pytorch_images128_ae.pkl')
            print('loading DAE image data from file: '+ images_path + ' ' + str(args.load))
            if args.load:
                self.ae_images_data = cPickle.load(open(images_path, 'rb'))
                for i in range(len(self.ae_images_data)):
                    self.ae_images_data[i] = my_preprocess(self.ae_images_data[i])
            else:
                self.ae_images_data = images_path
        # tokenization
    ...
    def __getitem__(self, index):
        ...
        if self.args.maml:
            if self.args.load:
                ...
                image_data[0] = self.maml_images_data
        if self.args.autoencoder:
            if self.args.load:
                ae_images_data = self.ae_images_data[entry['image']].reshape(128*128)
                # ae_images_data = self.ae_images_data[entry['image']].reshape(1024*1024)
                image_data[1] = ae_images_data
            else:
                image_data[1] = self.ae_images_data
...
def tfidf_from_questions(names, args, dictionary, dataroot='data', target=['rad']):
    ...
    # weights, _ = utils.create_glove_embedding_init(dictionary.idx2word[:N], glove_file)
    # np.save(os.path.join(args.VQA_dir, 'glove6b_init_300d.npy'), weights)
    # print('glove6b_init_300d (%d x %d) is generated.' % (weights.shape[0], weights.shape[1]))

    return tfidf, weights

if __name__=='__main__':
    dictionary = Dictionary.load_from_file('data_all_with_diff/dictionary.pkl')
    tfidf, weights = tfidf_from_questions(['train'], None, dictionary)
    w_emb = WordEmbedding(dictionary.ntoken, 300, .0, 'c')
    w_emb.init_embedding(os.path.join('data_all_with_diff', 'glove6b_init_300d.npy'), tfidf, weights)
    with open('data_all_with_diff/embed_tfidf_weights.pkl', 'wb') as f:
        torch.save(w_emb, f)
    print("Saving embedding with tfidf and weights successfully")

    dictionary = Dictionary.load_from_file('data_all_with_diff/dictionary.pkl')
    w_emb = WordEmbedding(dictionary.ntoken, 300, .0, 'c')
    with open('data_all_with_diff/embed_tfidf_weights.pkl', 'rb') as f:
        w_emb = torch.load(f)
