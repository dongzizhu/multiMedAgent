...
def compute_score_with_logits(logits, labels, debug=True):
    if debug:
        print(logits)
...
def get_answer(p, dataloader):
    _m, idx = p.max(1)
    return [dataloader.dataset.label2ans[x.item()] for x in idx]
...
def my_get_result(model, dataloader, device, args):
    ans_types = ['other', 'yes/no', 'all']
    keys = ['correct', 'total', 'score']
    result = dict((i, dict((j, 0.0) for j in keys)) for i in ans_types)
    answers_list = []
    ...
    with torch.no_grad():
        ...
        for v, q, a, ans_type, q_type, _ in iter(dataloader):
            if args.maml:
                v[0] = v[0].reshape(v[0].shape[0], 1, args.img_size, args.img_size)
            if args.autoencoder:
                v[1] = v[1].reshape(v[1].shape[0], 128, 128).unsqueeze(1)
            v[0] = v[0].to(device)
            v[1] = v[1].to(device)
            ...
            final_preds = preds
            if flag:
                batch_score = compute_score_with_logits(final_preds, a.data, debug=False).sum()
                tt += 1
                if tt == 2:
                  flag = False
            else:
            ...
            #   return result, answers_list
        print('time <s/sample>: ', total_time/result['all']['total'])
    if result['yes/no']['total'] > 0:
      result['yes/no']['score'] = result['yes/no']['correct'] / (result['yes/no']['total'] * args.batch_size)
    else:
      result['yes/no']['score'] = 0
    result['other']['score'] = result['other']['correct'] / (result['other']['total'] * args.batch_size)
    result['all']['score'] = result['all']['correct'] / len(dataloader.dataset)
    return result, answers_list
    ...
    def process(args, model, eval_loader):
        ...
        if args.use_VQA:
            if 'RAD' in args.VQA_dir:
                ...
                result, answers_list = my_get_result(model, eval_loader, args.device, args)
